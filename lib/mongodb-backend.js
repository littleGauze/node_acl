...
MongoDBBackend.prototype = {
  ...
  */
  ...
  /**
    Adds name mapping to db.
  */
  addNameMaps : function(transaction, bucket, type, values){
    contract(arguments)
        .params('array', 'string', 'string|number','object|array')
        .end();

    if(['role', 'resource'].indexOf(type) === -1) throw new Error("Type must be one of ['role', 'resource'].");
    var self=this;
    var updateParams = (self.useSingle? {_bucketname: bucket, type:type, key:values.key} : {type:type, key:values.key});
    var collName = (self.useSingle? aclCollectionName : bucket);
    transaction.push(function(cb){
      self.db.collection(self.prefix + self.removeUnsupportedChar(collName), function(err,collection){
        if(err instanceof Error) return cb(err);
        ...
        // update document
        collection.update(updateParams,{$set:values},{safe:true,upsert:true},function(err){
          if(err instanceof Error) return cb(err);
          cb(undefined);
        });
      });
    ...
    transaction.push(function(cb) {
      self.db.collection(self.prefix + self.removeUnsupportedChar(collName), function(err,collection){
        // Create index
        collection.ensureIndex({_bucketname: 1, type: 1}, function(err){
          if (err instanceof Error) {
            return cb(err);
          } else{
            cb(undefined);
  ...
  */
  delNameMap : function(transaction, bucket, type, keys){
    contract(arguments)
        .params('array', 'string', 'string', 'string|array')
        .end();
    keys = makeArray(keys);
    var self= this;
    var updateParams = (self.useSingle? {_bucketname: bucket, type:type, key:{$in:keys}} : {type:type, key:{$in:keys}});
    var collName = (self.useSingle? aclCollectionName : bucket);
