...
Acl.prototype.allow = function(roles, resources, permissions, cb){
  contract(arguments)
    .params('string|array|object','string|array|object','string|array|object','function')
    .params('string|array|object','string|array|object','string|array|object')
  ...
  if((arguments.length == 1) || ((arguments.length===2)&&_.isObject(roles)&&_.isFunction(resources))){
    ...
    var rawRoles = makeArray(roles);
    var rawResources = makeArray(resources);
    roles = makeArray(roles, true);
    resources = makeArray(resources, true);
    ...
    resources.forEach(function(resource, i){
      var res = null
      if (res = hasName(rawResources[i])) {
        _this.backend.addNameMaps(transaction, _this.options.buckets.namemap, 'resource', res);
      }
      roles.forEach(function(role){
    ...
    roles.forEach(function(role, i){
      var rs = null
      if (rs = hasName(rawRoles[i])) {
        _this.backend.addNameMaps(transaction, _this.options.buckets.namemap, 'role', rs);
      }
      resources.length && _this.backend.add(transaction, _this.options.buckets.resources, role, resources);
    });
...
Acl.prototype.removePermissions = function(role, resources, permissions, cb){
  ...
  resources.forEach(function(resource){
    ...
    if(permissions){
      ...
      _this.backend.del(transaction, bucket, role);
      _this.backend.delNameMap(transaction, _this.options.buckets.namemap, 'resource', resource);
      _this.backend.remove(transaction, _this.options.buckets.resources, role, resource);
    }
  });
...
*/
Acl.prototype.koaMiddleware = function({ numPathComponents, userId, actions, useRes } = {}){
  contract(arguments)
    .params()
    .params('object')
    .end();
  ...
  var acl = this;
  var mdw = (req, res) => {
    var fn = acl.middleware(numPathComponents, userId, actions);
  ...
  return async function(ctx, next){
    var reqObj = {};
    if (ctx.session) {
      reqObj.session = ctx.session;
    }
    if (ctx.state.user) {
      reqObj.user = ctx.state.user;
    }
    reqObj.originalUrl = useRes || ctx.originalUrl;
    reqObj.method = ctx.method;

    await mdw(reqObj, ctx.res);
    await next()
  };
};
...
function makeArray(arr, parse = false){
  var arr = Array.isArray(arr) ? arr : [arr]
  if (!parse) return arr;
  return arr.map(it => {
    if (typeof it === 'object') {
      return it.key
...
function hasName(doc) {
  return typeof doc === 'object' && doc.name ? doc : null;
}
